 ...
 */
class Lumia_Controller_Plugin_RestHandler extends Zend_Controller_Plugin_Abstract
{

	protected $dispatcher;
	protected $defaultFormat = 'html';
	protected $reflectionClass = null;
	protected $acceptableFormats = array('html', 'xml', 'php', 'json');
	...
	protected $requestTypes = array(
			Lumia_Form::ENCTYPE_MULTIPART, 
			Lumia_Form::ENCTYPE_URLENCODED, 
			'text/xml', 
			'application/xml', 
			'text/php', 
			'application/php', 
	 ...
	 */
	protected function setResponseFormat(Zend_Controller_Request_Abstract $request)
	{
		$format = false;
		
		// check query string first
		if (in_array($request->getParam('format', 'none'), $this->acceptableFormats))
		{
		...
		if ($format === false || ! in_array($format, $this->acceptableFormats))
		{
			...
			{
				$request->dispatchError(Lumia_Controller_Response_REST::UNSUPPORTED_TYPE, 'Unsupported Media/Format Type');
			}
		} else
		{
	 ...
	 */
	protected function handleActions(Zend_Controller_Request_Abstract $request)
	{
		...
		{
			if ($method->getDeclaringClass()->name != 'Lumia_Controller_REST')
			{
				$name = strtoupper($method->name);
				if ($name == '__CALL' && $method->class != 'Zend_Controller_Action')
				{
		...
		{
			$request->dispatchError(Lumia_Controller_Response_REST::NOT_ALLOWED, 'Method Not Allowed');
	 ...
	 * this parses the reqest body in accordance with RFC2616 spec regardless of
	 * the HTTP method
	 */
	protected function handleRequestBody(Zend_Controller_Request_Abstract $request)
	{
		...
		if (in_array($contentType, array(Lumia_Form::ENCTYPE_MULTIPART, Lumia_Form::ENCTYPE_URLENCODED)))
		{
			// PHP takes care of everything for us in this case lets just modify the $_FILES array
			if ($request->isPost() && $contentType == Lumia_Form::ENCTYPE_MULTIPART)
			{
		...
		} elseif (! empty($rawBody))
		{
			...
			} catch (Lumia_Exception_REST $e)
			{
				$request->dispatchError(Lumia_Controller_Response_REST::BAD_REQUEST, 'Invalid Payload Format');
				return;
			}
		}
	}
	 ...
	 */
	protected function getReflectionClass(Zend_Controller_Request_Abstract $request)
	{
	 ...
	 */
	protected function isRestClass($class)
	{
		if ($class === false)
		{
			return false;
		...
		if (in_array($class->name, array('Zend_Rest_Controller', 'Lumia_Controller_REST')))
		{
			return true;
		}
	...
	/**
	 * utility function to replace http_negotiate_content_type when its not
	 * available
	 * see: http://pecl.php.net/pecl_http
	 */
	protected function negotiateContentType($request)
	{
		...
		foreach ($types as $type)
		{
			...
			if (array_key_exists($type, $this->responseTypes) && ! array_key_exists($quality, $mimeTypes))
			{

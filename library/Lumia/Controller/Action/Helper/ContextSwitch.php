...
/**
 * Lumia_Controller_Action_Helper_ContextSwitch
 *
 * @category   Lumia
 * @package    Lumia_Controller_Action_Helper
 */
class Lumia_Controller_Action_Helper_ContextSwitch extends Zend_Controller_Action_Helper_ContextSwitch
{

	protected $_autoSerialization = true;
	protected $_availableAdapters = array(
			'json' => 'Zend_Serializer_Adapter_Json', 
			'xml' => 'Lumia_Serializer_Adapter_REST_Xml', 
			'php' => 'Zend_Serializer_Adapter_PhpSerialize'
	);
	protected $_restContexts = array(
			'json' => array(
					'suffix' => 'json', 
					'headers' => array('Content-Type' => 'application/json'), 
					'options' => array('autoDisableLayout' => true), 
					'callbacks' => array(
			...
			'html' => array(
					...
					'options' => array('autoDisableLayout' => false)
			)
	);
	...
	public function __construct($options = null)
	{
		...
		{
			$this->addContexts($this->_restContexts);
		}
	...
	public function getAutoDisableLayout()
	{
		...
		return $this->_restContexts[$context]['options']['autoDisableLayout'];
	}
		...
		if ($view instanceof Zend_View_Interface)
		{
			if (method_exists($view, 'getVars'))
			{
				...
				if (count($data) !== 0)
				{
					...
					switch (strtolower($this->_currentContext))
					{
						case 'xml':

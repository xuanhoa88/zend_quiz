...
class Lumia_Image
{
	 ...
	 */
	public static function factory ($filename = null, array $options = array(), $isDataStream = false)
	{
		...
		$pt = Lumia_Image_PhpThumb::getInstance();
		
		// load the plugins
		$pt->loadPlugins(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'Image' . DIRECTORY_SEPARATOR . 'Plugin');
		...
		{
			$imp = 'Lumia_Image_' . $implementationMap[$implementation];
		...
		{
			$imp = 'Lumia_Image_' . $implementationMap['gd'];
			$implementation = 'gd';
			$toReturn = new $imp($filename, $options, $isDataStream);
		}
		...
		{
			throw new Lumia_Image_Exception('You must have either the GD or iMagick extension loaded to use this library');
		}

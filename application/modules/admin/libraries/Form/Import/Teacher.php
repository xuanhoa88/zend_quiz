<?php
class Admin_Form_Import_Teacher extends Admin_Form_Import
{
	/**
	 * View script path
	 *
	 * @var	string
	 */
	protected $_viewScript = 'import/form/teacher.phtml';
	...
	{
		$this->setAction($this->getView()->baseUrl('admin/import/teacher'));
		...
		$imexRows = $imexModel->allActivate(strtoupper(Admin_Const::ROLE_CODE_TEACHER));
		if ($imexRows->count())
		{
    ...
    public function whenUploadSucceeded($filePath)
    {
		...
		$submit->setOptions(array(
				...
				'type' => 'button',
				'onClick' => 'LumiaJS.admin.import.processingTeacher(\'' . $filePath . '\'); return false;',
				'decorators' => array('ViewHelper')
		));
